...
def calculate_calories_burned(gender: int, weight: float, height: int, age: np.float64,
    ...
    """
    Рассчитать количество калорий, сожжённых во время сна.

    Описание:
    - Сначала рассчитывается BMR по формуле Миффлина-Джеора (с учётом пола).
    - Затем BMR переводится в энергозатраты за время сна: (BMR * sleep_hours) / 24.

    Параметры:
    - gender: int — 1 для мужчин, 0 для женщин.
    - weight: float — вес в килограммах.
    - height: int — рост в сантиметрах.
    - age: np.float64 — возраст в месяцах (в этой кодовой базе возраст хранится в месяцах).
    - sleep_duration: int — длительность сна в минутах.

    Возвращает:
    - float — калории, округлённые до одного знака после запятой.
    """
    bmr = 10 * weight + 6.25 * height - 5 * age / 12 + 5 if gender else 10 * weight + 6.25 * height - 5 * age / 12 - 161
    calories_burned = (bmr * (int(sleep_duration) / 60)) / 24
    return round(calories_burned, 1)


def evaluate_bedtime(sleep_data: SleepRecord) -> datetime:
    """
    Выбирает время укладывания в кровать между показаниями устройства и пользовательским временем.

    Логика:
    - Если `device_bedtime` раньше или равен `bedtime`, возвращаем `device_bedtime`.
    - Иначе — `bedtime`.
    """
...
def evaluate_wake_time(sleep_data: SleepRecord) -> datetime:
    """
    Выбирает время пробуждения между показаниями устройства и пользовательским временем.

    Логика:
    - Если `device_wake_up_time` позже или равен `wake_up_time`, возвращаем `device_wake_up_time`.
    - Иначе — `wake_up_time`.
    """
...
def chronotype_assessment(sleep_records: list) -> dict:
    """
    Оценивает хронотип пользователя по последовательности ночных записей.

    Алгоритм (упрощённо):
    - Для каждой записи вычисляется «midpoint» (середина сна) на основе времени начала и длительности.
    - Отбираются только свободные дни (weekend: weekday >= 5) — используются как основа для оценки.
    - Вычисляется MSF (mid-sleep on free days) и коррекция MSFsc.
    - Передаёт скорректированное время в `interpret_chronotype` для получения описательной категории.

    Возвращает словарь из `interpret_chronotype` или пустой словарь при недостатке данных.
    """
...
def time_to_minutes(dt, ref_hour=20):
    """
    Преобразует объект времени/даты в нормализованные минуты относительно `ref_hour`.

    Параметры:
    - dt: datetime | time — время или datetime-объект.
    - ref_hour: int — час (0-23), относительно которого нормализуем (по умолчанию 20 — 20:00).

    Возвращает:
    - int или None — количество минут, нормализованное к диапазону [0, 1440), или `None`, если dt пустой.
    """
    if not dt:
        return None
    hours = dt.hour
    ...
    normalized_minutes = (total_minutes - ref_minutes) % 1440
    return normalized_minutes


def sleep_regularity(sleep_records: list) -> dict:
    """
    Оценивает регулярность сна по набору записей.

    Возвращает словарь с `bedtime_std` и `wake_time_std` — стандартные отклонения в минутах.
    """
    if not sleep_records:
        return {}
...
def calculate_sleep_statistics_metrics(sleep_data: SleepRecord, age: np.float64, gender: int, weight: float,
                                    height: int) -> dict:
    """
    Вычисляет основные метрики сна на основе переданной записи `sleep_data`.

    Возвращаемые метрики:
    - `latency_minutes`, `sleep_efficiency`, `sleep_phases`, `sleep_fragmentation_index`, `sleep_calories_burned`.
    """
    if not sleep_data:
        return {}
    total_bedtime = evaluate_bedtime(sleep_data)
    total_wake_time = evaluate_wake_time(sleep_data)
    latency_delta = sleep_data.segments.order_by('start_time').values_list('start_time', flat=True).first() - total_bedtime
    ...
    sleep_phases = {
        'deep': sleep_data.sleep_deep_duration / (sleep_data.duration + sleep_data.sleep_awake_duration) * 100 if sleep_data.duration else 0,
        'light': sleep_data.sleep_light_duration / (sleep_data.duration + sleep_data.sleep_awake_duration) * 100 if sleep_data.duration else 0,
        'rem': sleep_data.sleep_rem_duration / (sleep_data.duration + sleep_data.sleep_awake_duration) * 100 if sleep_data.duration else 0,
        'awake': sleep_data.sleep_awake_duration / (sleep_data.duration + sleep_data.sleep_awake_duration) * 100 if sleep_data.duration else 0,
    }
    interpret_fragmentation = (sleep_data.awake_count / (sleep_data.duration / 60)) if sleep_data.duration else 0
    sleep_calories_burned = calculate_calories_burned(gender=gender, weight=weight, height=height, age=age, sleep_duration=sleep_data.duration)
    return {
